...
import { neutralize } from "./neutral";
...
export function describeError(err: unknown): string {
  ...
  switch (e.code) {
    ...
    case "P0001": return neutralize(humanizeStatuses(msg));                 // our workflow rules
  ...
  if (msg && msg.length < 200 && !/^\w+Error$/.test(msg)) return neutralize(msg);
